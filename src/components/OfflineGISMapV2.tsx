...
export function OfflineGISMapV2() {
  ...
  const refresh = async () => {
    const next = await offlineMapEngine.refresh(); setStatus(next);
    if (next?.labels) { const raw = await window.rnmsOffline?.readMapText?.('pakistan-labels.geojson'); try { setLabels(raw ? JSON.parse(raw) : null); } catch { setLabels(null); } } else setLabels(null);
  };
  useEffect(() => { void refresh(); }, []);
  const install = async () => { setBusy(true); try { await offlineMapEngine.installPackage(); await refresh(); } finally { setBusy(false); } };
  const tileUrl = layer === 'none' ? null : `rnms://tiles/${layer}/{z}/{x}/{y}.jpg`;
  const available = layer === 'none' || Boolean(status?.[layer]);
  return <div className="relative h-full w-full overflow-hidden bg-slate-200 dark:bg-slate-950">
    <MapContainer center={center} zoom={5} minZoom={3} maxZoom={18} className="h-full w-full">
      {tileUrl && available && <TileLayer url={tileUrl} maxZoom={18} keepBuffer={3} />}
    ...
    <div className="absolute top-4 left-4 z-[1000] flex flex-wrap gap-2">
      <div className="bg-slate-950/95 text-white rounded-xl px-3 py-2 text-xs border border-emerald-500/40"><b><WifiOff className="inline w-4 h-4 text-emerald-400 mr-1" />OFFLINE GIS ENGINE</b><div className="text-[10px] text-slate-400">Folder tiles · no PMTiles</div></div>
    ...
    <div className="absolute top-4 right-4 z-[1000] bg-white/95 dark:bg-slate-900/95 rounded-xl shadow-xl p-3 w-72 text-xs">
      ...
      <div className="text-[10px] text-slate-500 mb-2">Architecture: <b>Folder Tiles</b></div>
      <div className="grid grid-cols-2 gap-2">{([['satellite','Satellite'],['street','Road map'],['terrain','Terrain'],['labels','Place labels']] as const).map(([k,t]) => <div key={k} className="bg-slate-50 dark:bg-slate-800 rounded p-2"><div className="text-[9px] uppercase text-slate-400">{t}</div><b className={status?.[k] ? 'text-emerald-600' : 'text-rose-600'}>{status?.[k] ? 'READY' : 'MISSING'}</b></div>)}</div>
      ...
      <div className="mt-2 text-[9px] text-slate-400">Expected: tiles/satellite, tiles/street, tiles/terrain, pakistan-labels.geojson, metadata.json</div>
    </div>
  </div>;
}
