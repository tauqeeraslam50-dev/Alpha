...
import {
  ...
  Download, Upload, Settings, Sun, Moon, Info, Shield, Sparkles, User, Eye
} from 'lucide-react';
...
export function Sidebar() {
  ...
  const NavItem = ({ view, icon: Icon, label }: SidebarItemProps) => {
    ...
    return (
      <button
        ...
        className={cn(
          "flex items-center w-full px-3 py-2 text-xs font-semibold transition-all rounded-lg mb-1 text-left group",
          isActive 
            ? theme === 'light'
              ? "bg-blue-50 text-blue-700 shadow-sm border border-blue-200/80 font-bold"
  ...
  return (
    <nav className={cn(
      "w-64 flex flex-col flex-shrink-0 h-full relative z-20 border-r transition-colors duration-200",
      ...
      <div className="p-3.5 space-y-0.5 overflow-y-auto flex-1">
        ...
        <div className={cn(
          "px-3 text-[10px] font-bold uppercase tracking-wider mb-1.5 mt-5",
      ...
      {/* Developer & Software Attribution Card */}
      <div className={cn(
        "p-3.5 border-t",
        theme === 'light' ? "bg-slate-50/80 border-slate-200" : "bg-slate-950/60 border-slate-800/80"
      )}>
        <div 
          onClick={() => setIsAboutModalOpen(true)}
          className={cn(
            "p-2.5 rounded-xl border cursor-pointer transition-all hover:scale-[1.01]",
            theme === 'light' 
        ...
        >
          <div className="flex items-center justify-between mb-1">
            ...
            <Info className="w-3.5 h-3.5 text-blue-600 hover:text-blue-700" />
          </div>
          <div className={cn(
        ...
        <div className="flex items-center justify-between mt-2.5 px-1">
          <button
            onClick={toggleTheme}
            className={cn(
              "flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-md border transition-colors",
              theme === 'light'
...
export function Header() {
  ...
  return (
    <header className={cn(
      "h-14 border-b px-5 flex items-center justify-between flex-shrink-0 transition-colors duration-200 z-30 shadow-xs",
      theme === 'light' ? "bg-white border-slate-200" : "bg-slate-900 border-slate-800"
    )}>
      {/* App Branding */}
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline-block px-1.5 py-0.2 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 text-[10px] font-mono font-bold rounded">
              v1.0
            </span>
          </div>
          <p className="text-[10px] text-slate-400 leading-none mt-0.5 hidden sm:block">
            Develop by <strong className="text-slate-600 dark:text-slate-300 font-semibold">Tauqeer Aslam</strong>
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <div className={cn(
          "hidden md:flex items-center gap-2 px-2.5 py-1 rounded-md border text-xs font-semibold",
          ...
          <span className="font-mono text-blue-600 dark:text-blue-400">Region-7_VHF_Expansion</span>
        </div>
        ...
        <button 
          ...
          className={cn(
            "px-3 py-1.5 border text-xs font-semibold rounded-lg transition flex items-center shadow-xs",
            theme === 'light'
        ...
        >
          <Upload className="w-3.5 h-3.5 mr-1.5 text-emerald-600 dark:text-emerald-500" />
        ...
        <button 
          ...
          className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg shadow-xs transition flex items-center"
        >
        ...
        <button 
          onClick={() => setIsAboutModalOpen(true)}
          className={cn(
            "p-1.5 rounded-lg border transition ml-0.5",
            theme === 'light'
          ...
          title="About Software Version 1.0"
        >
...
export function StatusBar() {
  ...
  return (
    <footer className={cn(
      "h-8 px-5 flex items-center justify-between flex-shrink-0 text-[10px] font-mono border-t z-30 transition-colors duration-200",
      theme === 'light' ? "bg-white border-slate-200 text-slate-600" : "bg-slate-900 border-slate-800 text-slate-400"
    )}>
      <div className="flex items-center gap-5">
        <div className="flex items-center gap-1.5">
          <div className="h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.6)]"></div>
          <span className="font-semibold text-emerald-600 uppercase tracking-tight">System v1.0 Online</span>
        </div>
        <div className="hidden md:flex items-center gap-4 text-slate-500">
          <span>Sites: <b className={theme === 'light' ? "text-slate-800" : "text-slate-200"}>{sites.length}</b></span>
          <span>Links: <b className={theme === 'light' ? "text-slate-800" : "text-slate-200"}>{links.length}</b></span>
          <span>Frequency: <b className={theme === 'light' ? "text-slate-800" : "text-slate-200"}>136 - 470 MHz</b></span>
        </div>
      </div>

      <div className="flex items-center gap-4">
        <div className="hidden sm:flex items-center gap-3 font-bold uppercase text-[9px]">
          <span className="text-emerald-600">{goodLinks} Valid</span>
          <span className="text-amber-500">{marginalLinks} Marg</span>
          <span className="text-rose-500">{failedLinks} Fail</span>
        </div>

        <div className="border-l border-slate-300 dark:border-slate-700 pl-3 flex items-center gap-1 text-slate-500">
          <span>Dev:</span>
          <button 
            onClick={() => setIsAboutModalOpen(true)}
