...
import { Mountain, Satellite, Map as MapIcon, Database, WifiOff, RefreshCw, Archive, CheckCircle2, AlertTriangle, Globe2, Route } from 'lucide-react';
...
import { MapLibreOfflineMap } from './MapLibreOfflineMap';

type LayerMode = 'street' | 'satellite' | 'terrain' | 'none';
...
export function OfflineGISMap() {
  ...
  const streetUrl = useMemo(() => {
    const base = mapInfo?.pmtilesBaseUrl || 'http://127.0.0.1:39777/pmtiles/';
    return `${base}pakistan-streets.pmtiles`;
  }, [mapInfo]);

  useEffect(() => {
    let cancelled = false;
    if (!mapInfo?.pmtilesBaseUrl) return;
    fetch(streetUrl, { method: 'HEAD', cache: 'no-store' })
      .then(r => { if (!cancelled) setStreetReady(r.ok); })
      .catch(() => { if (!cancelled) setStreetReady(false); });
    return () => { cancelled = true; };
  }, [streetUrl, mapInfo?.pmtilesBaseUrl]);
  ...
  return <div className="relative h-full w-full overflow-hidden bg-slate-200 dark:bg-slate-950">
    {activeOffline && layer === 'street' && streetReady && (
      <MapLibreOfflineMap vectorUrl={streetUrl} center={[69.3451, 30.3753]} sites={sites as any} links={links as any} />
    )}

    {(!activeOffline || layer !== 'street') && (
      <MapContainer center={center} zoom={layer === 'satellite' ? 10 : 5} minZoom={0} maxZoom={18} className="h-full w-full z-0" zoomControl>
        {activeOffline && layer === 'satellite' && satellitePMTiles && <PMTilesLayer url={satellitePMTiles} minZoom={8} maxZoom={15} attribution="Offline satellite imagery · PMTiles" />}
    ...
    <div className="absolute top-4 left-4 z-[1000] flex flex-wrap gap-2 max-w-[calc(100%-2rem)]">
      <div className="bg-slate-950/95 text-white rounded-xl shadow-xl border border-emerald-500/40 px-3 py-2 text-xs"><div className="font-bold flex items-center gap-2"><WifiOff className="w-4 h-4 text-emerald-400" /> OFFLINE-FIRST GIS</div><div className="text-[10px] text-slate-400 mt-1">MapLibre + Shortbread PMTiles + real DEM</div></div>
      <button onClick={() => { setOnlineMode(false); setLayer('street'); }} className={`px-3 py-2 rounded-xl text-xs font-bold shadow border ${activeOffline && layer === 'street' ? 'bg-emerald-600 text-white border-emerald-500' : 'bg-white text-slate-700 border-slate-200'}`}><Route className="inline w-3.5 h-3.5 mr-1" />Street Map</button>
      <button onClick={() => { setOnlineMode(false); setLayer('satellite'); }} className={`px-3 py-2 rounded-xl text-xs font-bold shadow border ${activeOffline && layer === 'satellite' ? 'bg-blue-600 text-white' : 'bg-white text-slate-700 border-slate-200'}`}><Satellite className="inline w-3.5 h-3.5 mr-1" />Satellite</button>
      <button onClick={() => { setOnlineMode(false); setLayer('terrain'); }} className={`px-3 py-2 rounded-xl text-xs font-bold shadow border ${activeOffline && layer === 'terrain' ? 'bg-emerald-600 text-white' : 'bg-white text-slate-700 border-slate-200'}`}><Mountain className="inline w-3.5 h-3.5 mr-1" />Terrain</button>
    ...
    <div className="absolute bottom-4 left-4 z-[1000] bg-white/95 dark:bg-slate-900/95 border border-slate-300 dark:border-slate-700 rounded-xl shadow-xl p-3 w-96 max-w-[calc(100%-2rem)] text-xs">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-lg bg-slate-50 dark:bg-slate-800/70 p-2"><div className="text-[10px] uppercase text-slate-400 font-bold">Street PMTiles</div><b className={streetReady ? 'text-emerald-600' : 'text-rose-600'}>{streetReady ? 'READY' : 'MISSING'}</b></div>
      ...
      {layer === 'street' && !streetReady && <div className="mt-2 border-t pt-2 text-amber-700 dark:text-amber-300"><Archive className="inline w-3.5 h-3.5 mr-1" />Put the downloaded Shortbread PMTiles at <code>rnms-data/maps/pakistan-streets.pmtiles</code>.</div>}
      ...
      {demInfo?.unavailable && <div className="mt-2 border-t pt-2 text-amber-700 dark:text-amber-300">No real DEM tile covers this coordinate.</div>}
