...
type GISInfo = {
  ...
  satellite: AssetInfo | null;
  terrain: AssetInfo | null;
  demTileCount: number;
  demTiles: string[];
  demResolution: string | null;
  pmtilesBaseUrl?: string;
};
declare global { interface Window { rnmsOffline?: any } }

const emptyInfo: GISInfo = {
  mapsRoot: '', demRoot: '', satelliteAvailable: false, terrainAvailable: false,
  satellite: null, terrain: null, demTileCount: 0, demTiles: [], demResolution: null
};
...
export function OfflineMapManager() {
  ...
  const refresh = useCallback(async () => {
    setBusy(true);
    try {
      const result = await window.rnmsOffline?.getMapInfo?.();
  ...
  const installMaps = async () => {
    setBusy(true);
    setMessage('Select one or more PMTiles archives. Use names containing satellite/imagery or terrain so the application can classify them.');
    try {
      ...
      setMessage(`Installed ${result.installed?.length || 0} archive(s). Skipped ${result.skipped?.length || 0}.`);
  ...
  const installDem = async () => {
    setBusy(true);
    setMessage('Select the folder containing genuine SRTM/HGT files.');
    try {
  ...
  const validate = async () => {
    setBusy(true);
    try {
      const result = await window.rnmsOffline?.validateAssets?.();
      setValidation(result ?? { valid: false, warnings: ['Electron GIS bridge unavailable.'] });
      if (result) setInfo(result);
    } finally { setBusy(false); }
  };
  ...
  const remove = async (name: string) => {
    ...
    setBusy(true);
    try {
      await window.rnmsOffline?.removeMapAsset?.(name);
  ...
  return <div className={`p-4 md:p-6 space-y-5 h-full overflow-y-auto ${theme === 'light' ? 'text-slate-800' : 'text-slate-100'}`}>
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 rounded-xl bg-blue-600 text-white flex items-center justify-center"><Layers3 className="w-6 h-6" /></div>
        <div><h2 className="text-xl font-bold">Offline GIS Data Manager</h2><p className="text-xs text-slate-500">Manage local satellite, terrain and real HGT elevation data without rebuilding the application.</p></div>
      </div>
    ...
    <section className="rounded-xl border border-blue-200 dark:border-blue-900/50 bg-blue-50 dark:bg-blue-950/20 p-4 flex gap-3">
      <Info className="w-5 h-5 shrink-0 text-blue-600 mt-0.5" />
      <div><b className="text-sm">Current GIS status</b><p className="text-xs text-slate-600 dark:text-slate-300 mt-1">{coverageNote}</p></div>
    </section>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <section className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 space-y-4">
        ...
        <p className="text-xs text-slate-500">Import your large map archives into the external GIS data store. The executable stays small and the datasets can be replaced later.</p>
    ...
    <section className="rounded-xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/20 p-5">
      <h3 className="font-bold flex items-center gap-2"><Map className="w-5 h-5 text-amber-600" />Vector map and city labels</h3>
      <p className="text-xs text-slate-600 dark:text-slate-300 mt-2">Satellite imagery contains pixels, not city/place names. For the final offline map we need a separate vector tile dataset containing places, roads and boundaries. Keep that dataset in the same GIS data area; it should be loaded above the satellite layer by the map renderer.</p>
      <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-2 text-xs"><div className="rounded-lg bg-white/70 dark:bg-slate-900/50 p-3"><b>Satellite</b><br />Imagery background</div><div className="rounded-lg bg-white/70 dark:bg-slate-900/50 p-3"><b>Vector</b><br />Cities, roads, places</div><div className="rounded-lg bg-white/70 dark:bg-slate-900/50 p-3"><b>DEM</b><br />Real elevation / LOS</div></div>
    </section>

    <section className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3"><div><h3 className="font-bold flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-amber-600" />GIS Integrity Check</h3><p className="text-xs text-slate-500">Checks installed archives and HGT dimensions before they are used by the GIS/LOS system.</p></div><button disabled={busy} onClick={validate} className="px-4 py-2 rounded-lg bg-slate-800 text-white text-xs font-bold"><Database className="inline w-4 h-4 mr-1" />Validate Installed Data</button></div>
      {validation && <div className={`rounded-lg p-3 text-xs ${validation.valid ? 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300' : 'bg-amber-50 text-amber-800 dark:bg-amber-950/30 dark:text-amber-300'}`}><b>{validation.valid ? 'Validation passed.' : 'Validation warnings:'}</b>{validation.warnings.length ? <ul className="list-disc ml-5 mt-1">{validation.warnings.map((w, i) => <li key={i}>{w}</li>)}</ul> : ' No integrity warnings detected.'}</div>}
      {message && <div className="text-xs rounded-lg border border-blue-200 bg-blue-50 dark:bg-blue-950/20 p-3">{message}</div>}
    </section>
