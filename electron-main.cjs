...
function registerOfflineGIS() {
  protocol.handle('rnms', async (request) => {
    ...
    if (parts[0] !== 'tiles' || parts.length !== 5) return new Response('Not found', { status: 404 });
    const layer = parts[1].replace(/[^a-z0-9_-]/gi, '');
    const z = parts[2].replace(/\D/g, '');
    const x = parts[3].replace(/\D/g, '');
    const file = parts[4].replace(/[^a-z0-9_.-]/gi, '');
    if (!z || !x || !file || !['jpg', 'jpeg', 'png', 'webp'].includes(path.extname(file).slice(1).toLowerCase())) {
  ...
  ipcMain.handle('offline-map-info', async () => {
    ...
    return {
      mapsRoot: roots.maps,
      demRoot: roots.dem,
      satelliteAvailable: fs.existsSync(path.join(roots.maps, 'tiles', 'satellite')),
      terrainAvailable: fs.existsSync(path.join(roots.maps, 'tiles', 'terrain')),
  ...
  ipcMain.handle('offline-dem-tile', async (_event, tileName) => {
    ...
    const roots = getAssetRoots();
    const file = safeChild(roots.dem, tileName);
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    minWidth: 1024,
    minHeight: 700,
    title: 'Radio Network Management System v1.0 (Offline GIS Foundation)',
  ...
  const template = [
    { label: 'File', submenu: [
      ...
      { type: 'separator' }, { label: 'Exit', accelerator: 'CmdOrCtrl+Q', click: () => app.quit() }
    ]},
    { label: 'View', submenu: [
      { role: 'resetZoom' }, { role: 'zoomIn' }, { role: 'zoomOut' }, { type: 'separator' },
      { role: 'togglefullscreen' }, { label: 'Toggle Developer Tools', accelerator: 'CmdOrCtrl+Shift+I', click: () => mainWindow.webContents.toggleDevTools() }
    ]},
    { label: 'Offline GIS', submenu: [
      { label: 'Satellite tiles: local only', enabled: false },
      { label: 'DEM: SRTM/HGT local only', enabled: false },
      { label: 'LOS terrain: DEM-backed when tiles installed', enabled: false }
    ]},
    { label: 'Help', submenu: [{ label: 'About RNMS v1.0', click: () => require('electron').dialog.showMessageBox(mainWindow, {
      type: 'info', title: 'Radio Network Management System v1.0',
